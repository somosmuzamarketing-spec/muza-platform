...
export async function nominateMentora(_prev: NominationResult, formData: FormData): Promise<NominationResult> {
  try {
    ...
    if (!topic || !message) return { error: "Completa tu área de expertise y tu motivación." };

    await prisma.nomination.create({ data: { userId, type: "MENTORA", topic, message } });
    return { ok: true };
  } catch (e: any) {
