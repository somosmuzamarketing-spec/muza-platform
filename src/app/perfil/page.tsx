...
function renderBio(bio: string) {
  ...
  return (
    <div className="profile-bio">
      {sections.map((s, i) => {
        const isHighlight = /mensaje a las muzas/i.test(s.label);
        return (
          <div key={i} className={isHighlight ? "profile-bio-highlight" : "profile-bio-section"}>
            <p className="profile-bio-label">{s.label}</p>
            <p className="profile-bio-text">{s.text}</p>
          </div>
        );
      })}
    </div>
